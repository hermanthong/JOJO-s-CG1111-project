...
#define MIN_DISTANCE_LEFT 730
#define MIN_DISTANCE_RIGHT 800
...
void loop() { // Every loop will increase the timer by one
  delay(AVOID_WALL/4);
  ...
  // Line sensor check
  // If the bot senses a black strip, do the colour challenge and
  // reset the timer to 0
  sensorState = lineFinder.readSensors();
  if (sensorState != S1_OUT_S2_OUT && timer >= 20){ 
    motor1.stop();
    motor2.stop();
    delay(DELAY_TIME);    
    colorChallenge();
  ...
  // IR sensor check
  if(inputLeft <= MIN_DISTANCE_LEFT && inputLeft > 150){
    turnRight(AVOID_WALL);    
  } else if (inputRight <= MIN_DISTANCE_RIGHT && inputRight > 150){
    turnLeft(AVOID_WALL);
  }else{
    moveForward();
  }
...
void colorChallenge() {
  ...
  int whiteArr[3] = {588, 352, 415};
  int blackArr[3] = {249, 135, 177};
